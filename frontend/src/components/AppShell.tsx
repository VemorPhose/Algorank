import { Code2, Crown, Home, LogOut, Menu, ScrollText, Shield, Trophy, UserRound, X } from "lucide-react";
...
function navClass({ isActive }: { isActive: boolean }) {
  return cn(
    "inline-flex items-center gap-2 border-2 px-3 py-2 text-sm font-black transition",
    isActive ? "border-ink bg-lemon text-ink shadow-block-sm" : "border-transparent text-ink hover:border-ink hover:bg-white",
  );
}
  ...
  return (
    <div className="min-h-screen bg-paper text-ink">
      <div className="fixed inset-0 -z-10 overflow-hidden">
        <div className="art-shape left-[4%] top-24 h-24 w-24 bg-aqua" />
        <div className="art-shape right-[6%] top-36 h-16 w-36 bg-lemon" />
        <div className="art-shape bottom-20 left-[12%] h-20 w-40 bg-coral" />
      ...
      <header className="sticky top-0 z-40 border-b-2 border-ink bg-paper/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
          <Link to="/" className="group flex items-center gap-3">
            <span className="grid h-11 w-11 place-items-center border-2 border-ink bg-coral text-xl font-black text-white shadow-block-sm transition group-hover:-translate-y-0.5">
              A
            </span>
            <span className="text-2xl font-black text-ink">Algorank</span>
          </Link>
        ...
        {open ? (
          <div className="border-t-2 border-ink bg-paper px-4 py-4 lg:hidden">
      ...
      <main className="mx-auto min-h-[calc(100vh-9rem)] max-w-7xl px-4 py-8 md:py-10">
        <Outlet />
      </main>

      <footer className="border-t-2 border-ink bg-ink px-4 py-6 text-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 text-sm font-bold md:flex-row md:items-center md:justify-between">
          ...
          <span className="text-white/70">FastAPI, Redis, PostgreSQL, Judge0, React</span>
        </div>
      </footer>
    </div>
  );
